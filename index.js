...
app.post('/transmit', upload.single('file'), async (req, res) => {
    const { incarnationId, type, size } = req.body;
    ...
    if (user[type] >= SUBMISSION_LIMIT) return res.status(403).json({ error: `[QUOTA EXCEEDED.]` });
    ...
    try {
        ...
        const payload = {
            embeds: [{
                ...
                color: 5814783,
                description: `**Incarnation:** \`${incarnationId.substring(0,8)}\`\n**Type:** ${type.toUpperCase()}\n**Count:** [${user[type] + 1}/${SUBMISSION_LIMIT}]`
            }]
        };
...
// ==========================================
// 3. FRONTEND (The Multi-Phase Interface)
// ==========================================
app.get('/', (req, res) => {
    res.send(`
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .window { 
            background: #3498db; width: 100%; max-width: 360px; 
        ...
        .details span { border-bottom: 1px solid white; }

        /* THE CROPPING OVERLAY */
        #crop-overlay {
        ...
        /* INPUTS & BUTTONS */
        select, input, .btn { width: 100%; background: rgba(0,0,0,0.1); border: 1px solid white; color: white; padding: 10px; margin-top: 8px; font-weight: bold; font-size: 0.8rem; border-radius: 0; }
...
<body>
    <div class="window" id="main-panel">
        <div class="glitch-tl"></div>
        <div class="glitch-br"></div>
        <div class="system-controls">_ ▢ X</div>
        ...
        <div id="initial-ui">
            <div class="details">
                CATEGORY: MAIN<br>
                DIFFICULTY: F<br>
                CLEAR CONDITION: TRANSMIT <span>BEST ASSETS</span><br>
                TIME LIMIT: 4 TO 7 DAYS<br>
                REWARDS: <span>SPECIAL ROLE</span><br>
                PENALTY FOR FAILURE: ???
            </div>
            <select id="type">
        ...
        <div id="preview-section">
            <p style="font-size: 0.8rem;">[PREVIEWING OFFERING]</p>
            <img id="preview-img" src="">
            <button class="btn" id="finalizeBtn">VALIDATE & TRANSMIT</button>
            <button class="btn secondary" onclick="location.reload()">RE-SELECT</button>
        </div>
    ...
    <div id="crop-overlay">
        <div class="crop-container">
            <p style="text-align:center; font-size: 0.8rem; margin-bottom: 10px;">[ADJUST ASSET PROBABILITY]</p>
    ...
    <script>
        ...
        // When user finishes cropping in the separate panel
        document.getElementById('confirmCropBtn').onclick = () => {
            ...
            croppie.result({ type: 'blob', size: { width: size, height: size } }).then(blob => {
                ...
                // Hide Initial Form, Show Review
                initialUI.style.display = 'none';
                previewSection.style.display = 'block';
                
                // Show Cropped Image Overview
        ...
        document.getElementById('finalizeBtn').onclick = () => {
            ...
            formData.append('size', document.getElementById('type').value === 'sticker' ? 320 : 128);
...
app.listen(PORT, () => {
    console.log("System Active: http://localhost:" + PORT);
});
